  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.status === 'success') {
        toast({ 
          title: "Message Sent", 
          description: "Thank you for your inquiry. We'll respond to you shortly.",
          variant: "default",
        });
        setFormData({ 
          fullName: "", 
        ...
        throw new Error(data.response || 'Failed to send message');
      }
    } catch (error) {
          ...
          <div className="grid gap-16 lg:grid-cols-12">
            ...
            >
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                >
                  {isSubmitting ? (
                    <>Sending <span className="ml-2 inline-block h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent" /></>
                  ) : (
